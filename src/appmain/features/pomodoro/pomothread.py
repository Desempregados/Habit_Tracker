...
class Perfil_pomodoro(QObject):
    ...
    def __init__(self, len_work:int = 25, len_short:int = 5, len_long:int = 15, len_cycle:int = 3, parent = None):
    ...
    def next_cycle(self):
        if (self.current_cycle == "Work") and (self.completed_cycles % self.len_cycle == 0):
            ...
            self.sinal_tempo.emit(self.form_time(self.left_time))
            self.completed_cycles += 1
            self.sinal_current_cycle.emit(f"{(self.completed_cycles -1) % self.len_cycle}/{self.len_cycle}")
    ...
    def pomo_profile_change(self, new_work_len, new_short_break, new_long_break, new_cycle_len):
...
if __name__ == "__main__":
    ...
    p1.sinal_tempo.connect(lambda tempo_str: print(f"GUI (simulada) display: {tempo_str}"))
    p1.sinal_ciclo_estado.connect(lambda ciclo: print(f"Sinal de ciclo concluído: {ciclo}"))
